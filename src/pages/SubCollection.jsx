...
function SubCollection(params) {
    ...
    useEffect(() => {
        const fetchProducts = async () => {
            ...
            try {
                const result = await axios.get(`${BASE_URL}/api/products/category/${slug1}/${slug2}`);
    ...
    const Item = (props) => {
        ...
        return (
            <Product product={productObject} loading={loading} onSale={onSaleValue} discountPercent={discountPercentValue} key={Math.random()} />
        )
    }
            ...
            <Row>
                ...
                <Col>
                    <GridSystem colCount={3} md={6}>
                        {
                            currentListedProducts.length > 0 ? currentListedProducts.map(item => <Item key={item.id} productObject={item} onSaleValue={false} discountPercent={0} />) : [<h1 key="1">No product here</h1>]
                        }
                    </GridSystem>
                </Col>
            </Row>
